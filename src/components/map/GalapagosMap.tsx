...
function MapEffect({ selectedIsland, onIslandClick }: GalapagosMapProps) {
  const map = useMap();
  useEffect(() => {
    if (selectedIsland) {
...
export default function GalapagosMap({ onIslandClick, selectedIsland }: GalapagosMapProps) {
    ...
    return (
        <MapContainer center={position} zoom={7} scrollWheelZoom={true} className="h-full w-full">
            ...
            />
            <MapEffect selectedIsland={selectedIsland} onIslandClick={onIslandClick} />
